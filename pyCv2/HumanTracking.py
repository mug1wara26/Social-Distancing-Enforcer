...
def display_frame(frame, innerframe, dimensions, corners, height, width):
    ...
    height, width, hMatrix = transformInfo(corners, height, width, 500)  # pass width, height where 5, 5 is
...
if __name__ == "__main__":
    ...
    height, width = 5, 5
    while True:
        transimage, dimensions = get_boundaries(cap, 0.01)
        ret, orimage = cap.read()

        frame = display_frame(orimage, transimage, dimensions, points, height, width)
        cv2.imshow("frame", cv2.cvtColor(frame, cv2.COLOR_BGR2RGB))
